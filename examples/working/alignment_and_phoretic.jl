...
function simulation(save_path)
    ...
    field_forces = (field_propulsion_distr_force(1,0.2,1.,0.4,3), self_align_with_∇C_force(1,3), grad_field_propulsion_force(1,0.,-3))
    ...
    sim = Euler_integrator(system,0.05, 500, Tplot= 20, fps=120, Tsave = nothing ,save_folder_path = save_path, save_functions=(save_2d_polar_p!,save_single_2d_field!),plot_functions=(plot_field_magnitude!,plot_disks_orientation!, plot_directors!, plot_velocity_vectors!), plotdim=2); 
    return sim

end


sim = simulation(joinpath(homedir(),"surfdrive","ActivePolygonClusters","simulations","align_phoretic","phi_0p1","simdata"))