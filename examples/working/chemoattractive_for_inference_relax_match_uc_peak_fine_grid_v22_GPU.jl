...
#Import here to avoid the need of 
using Metal
...
function simulation(save_path)
    ...
    # One R comes from lbin, the other for scaling the magnitudes of the active forces due to the speed
    field_forces =(field_propulsion_distr_force(1,5. *R^2 / lbin^2,log10(2),4.5*2.5,0.261), self_align_with_∇C_force(1,3 * R^2), grad_field_propulsion_force(1,0.,-10 *R^2))
    ...
    #initial_field_state=[GPUField2d(1,1,bin_centers,C, C.*0, C.*0, MtlArray(C), MtlArray(Cv), MtlArray(Cf), Float32(lbin))]
    initial_field_state=[GeneralField2d(1,1,bin_centers,C, C.*0, C.*0, lbin)]
    # [field_propulsion_3d_force(1,1e-2,0.01)]
    #field_updaters = (CPU_to_GPU(1),GPUDiffusion(1,R^2*1e1),GPURelax(1,1.,0.3))#Need GPU version of: , GhostSet(1))
    field_updaters = (Diffusion(1,R^2*1e1),Relax(1,1.,0.3),GhostSet(1))#Need GPU version of: , GhostSet(1))
    ...
    sim = Euler_integrator(system,0.0005 ,1, Tplot=nothing, fps=120, res=(1000,1000), Tsave = nothing ,save_folder_path = save_path, save_functions=(save_2d_polar_p!,),plot_functions=(plot_field_magnitude!, plot_disks_orientation!, plot_directors!), plotdim=2); 
    return sim

end

sim = simulation(joinpath(homedir(),"surfdrive","ActivePolygonClusters","simulations","for_inference_v22_fine_grid","phi_0p01","simdata"))

@profview simulation(joinpath(homedir(),"surfdrive","ActivePolygonClusters","simulations","for_inference_v22_exp_rep","phi_0p01","simdata"))

@profview_allocs  simulation(joinpath(homedir(),"surfdrive","ActivePolygonClusters","simulations","for_inference_v22_exp_rep","phi_0p01","simdata"))
