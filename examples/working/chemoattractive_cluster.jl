...
function simulation(save_path)
    ...
    sim = Euler_integrator(system,0.005 ,600, Tplot=nothing, fps=120, res=(1000,1000), Tsave = 20 ,save_folder_path = save_path, save_functions=(save_2d_polar_p!,),plot_functions=(plot_field_magnitude!, plot_disks_orientation!, plot_directors!), plotdim=2); 
    return sim

end

sim = simulation(joinpath(homedir(),"surfdrive","ActivePolygonClusters","simulations","clustering","phi_0p1","simdata"))


#sim = simulation(joinpath("/Volumes","T7_Shield","ActivePolygonClusters","simulations","for_inference_v23_fine_grid","phi_0p01","simdata"))


@profview  simulation(joinpath(homedir(),"surfdrive","ActivePolygonClusters","simulations","clustering","phi_0p1","simdata"))
