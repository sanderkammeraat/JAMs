...
function simulation(save_path)
    ...
    field_forces =(reaction_field_propulsion_distr_force(1,5. *R^2 / lbin^2,log10(2),4.5*2.5,0.261), self_align_with_∇C_force(1,3 * R), grad_field_propulsion_force(1,0.,-10 *R^2))
    ...
    sim = Euler_integrator(system,0.005 ,600, Tplot=20, fps=120, res=(1000,1000), Tsave = nothing ,save_folder_path = save_path, save_functions=(save_2d_polar_p!, save_single_2d_field_10!),plot_functions=(plot_field_magnitude!, plot_disks_orientation!, plot_directors!), plotdim=2); 
    return sim
