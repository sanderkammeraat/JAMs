...
function simulation(save_path)
    ...
    field_forces =(field_propulsion_distr_force(1,1e3,log10(2),4.5,0.261), self_align_with_∇C_force(1,1), grad_field_propulsion_force(1,0.,-10))
    ...
    sim = Euler_integrator(system,0.005 ,600, Tplot= nothing, fps=120, res=(1000,1000), Tsave = 20 ,save_folder_path = save_path, save_functions=(save_2d_polar_p!,save_single_2d_field!),plot_functions=(plot_field_magnitude!, plot_disks_orientation!, plot_directors!,plot_trajectories!), plotdim=2); 
    return sim

end


sim = simulation(joinpath(homedir(),"surfdrive","ActivePolygonClusters","simulations","for_inference_v19_exp_rep","phi_0p01","simdata"))

@profview simulation(joinpath(homedir(),"surfdrive","ActivePolygonClusters","simulations","for_inference_v17_exp_rep","phi_0p01","simdata"))
