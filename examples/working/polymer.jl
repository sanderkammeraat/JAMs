...
function simulation()
    ...
    kpar = 0.
    kper=-1.
    pair_forces = (polymer_exterior_soft_disk_force(1,1.),polymer_harmonic_bend_force(1,0.5), polymer_harmonic_stretch_force(1,1.,0.5),polymer_align_director_tangent_force(1,10), polymer_pairAN_force(1,true, true,1.5, kpar, kper, 0.2))
    ...
    sim = Euler_integrator(system,0.025, 1e4, fps=60, Tplot=20,plot_functions=(plot_polymers!, plot_nematic_directors!, plot_velocity_vectors!), plotdim=2, Tsave=nothing, save_functions=(save_2d_polymer_polar_p!,),save_folder_path = joinpath(homedir(),"ANP","demos","N_in_pol_$(N_in_pol)","k_par_$(kpar)","k_per_$(kper)")); 
    return sim;
