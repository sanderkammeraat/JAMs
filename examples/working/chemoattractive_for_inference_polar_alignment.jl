...
function simulation(save_path)
    ...
    R = 5.
    pair_forces =(exp_repulsion_force(1, 5. /R*R , 0.45), pair_polar_alignment_force(1,40,0.02))
    ...
    sim = Euler_integrator(system,0.005 ,600, Tplot=nothing, fps=120, res=(1000,1000), Tsave = 20 ,save_folder_path = save_path, save_functions=(save_2d_polar_p!, save_single_2d_field_10!),plot_functions=(plot_field_magnitude!, plot_disks_orientation!, plot_directors!), plotdim=2); 
    return sim

end

sim = simulation(joinpath(homedir(),"surfdrive","ActivePolygonClusters","simulations","for_inference_polar_alignment_v2","phi_0p01","simdata"))
