...
function simulation(save_path)
    ...
    field_forces =(field_propulsion_distr_force(1,1e2,1.,1.5/0.3,0.3), self_align_with_∇C_force(1,30.), grad_field_propulsion_force(1,0.,-2))
    ...
    sim = Euler_integrator(system,0.005, 600, Tplot= 10, fps=120, Tsave = nothing ,save_folder_path = save_path, save_functions=(save_2d_polar_p!,save_single_2d_field!),plot_functions=(plot_field_magnitude!, plot_disks_orientation!, plot_directors!), plotdim=2); 
    return sim

end


sim = simulation(joinpath(homedir(),"surfdrive","ActivePolygonClusters","simulations","for_inference_v6","phi_0p01","simdata"))
