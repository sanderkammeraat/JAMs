...
function simulation(v0, Dr, J, Tplot, Tsave,tend)
    ...
    sim = Euler_integrator(system,0.01,tend, Tsave=Tsave, fps=120,Tplot=Tplot,plot_functions=(plot_potential!,plot_trajectories!,), plotdim=2, save_folder_path = save_folder, save_functions = (save_2d_polar_p!,)); 
    return sim;
...
tend = 1e6 #for how long to run the simulation (in units of simulation time)
