...
function main(base_folder, animation_base_folder; raw_data_file_name="sa_raw_data.h5")
...
@views function make_movie(raw_data_file,save_folder)
    ...
    scatter!(ax,x,y, color=c, markersize =s,marker = Circle, markerspace=:data,alpha=1, strokecolor=:black, strokewidth=1,colormap=:hsv,colorrange=(-pi,pi))
...
make_movie(jldopen(joinpath(base_folder,"simdata","sa_raw_data.h5"),"r"),mkpath(animation_base_folder))
