...
@everywhere function self_aligning_step(rx_step,J, Dr,v0, seed,save_folder_path; Tsave=100, Tplot=nothing)
...
v0s = [0.,0.0001, 0.0005,0.001,0.002,0.005, 0.01, 0.02, 0.05, 0.1, 0.2, 0.5, 1.0] 
Js=[1.]

seeds = reshape( collect(1:length(v0s)*length(Js)), (length(v0s),length(Js)) ) .+1000

for j in eachindex(Js)
    @sync @distributed for i in eachindex(v0s)
