...
function relaxation_step(save_folder_path; Tsave=nothing, Tplot=nothing, Nlin=40)
...
function self_aligning_step(rx_step,J,v0, Dr, seed,save_folder_path; Tsave=nothing, Tplot=nothing)
    ...
    #Run integration
    sim = Euler_integrator(system,0.01, 1e4,Tsave=Tsave,seed=seed, save_functions = [save_2d_polar_p!],save_folder_path=save_folder_path,save_tag="sa", fps=60, plot_functions=(plot_disks_vx!,plot_directors!, plot_velocity_vectors!), plotdim=2, Tplot=Tplot)#,record_folder_path="/Users/kammeraat/sa_explore/",res=(1000,1000)); 
    return sim
end
...
sa_result=self_aligning_step(rx_result, 60 , 0.01, 0.01,nothing, "", Tplot=10,  Tsave=nothing);
