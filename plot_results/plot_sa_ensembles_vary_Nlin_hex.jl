...
f = Figure()
Drplot = 0.1
ax = Axis(f[1,1], xlabel=L"$\Delta t$", ylabel=L"$\langle \vec{v}(t+\Delta t)\cdot \vec{v}(t) \rangle$/(v_{rms}(t+\Delta t)v_{rms}(t))", title="Dr = $Drplot")
the_auto=[]
for e in ensemble_files
    ...
    if Dr==Drplot #&& J==0.1
        ...
        for i in eachindex(the_auto)
            ti = det[i]
            #the_auto_s[i] = exp(-ti/tauJ)*tauJ/tau
            the_auto[i] = sum(@.  (exp(-ti/tau) - eigvals * tau * exp(-ti* eigvals))/ (tau^3 * eigvals^2 - tau) )
        end
        
        lines!(ax, det , the_auto/the_auto[1], color="red", label="ABP_theory", linewidth=10)

        end
    scatterlines!(ax,det , e["auto_v"]["Cavg"], colorrange = (1, 13 ) , color=ind, label="J = $(e["J"])",alpha=1,colormap=:jet)
    ind+=1
...
f[1,2]=Legend(f,ax)
display(f)
save(joinpath(figure_save_folder,"auto_v_Dr_$Drplot.pdf"), f, backend=CairoMakie)
...
for e in ensemble_files
    ...
    if Dr==0.1 && J>0
       ...
       # theory_integral_2 = theory_integral_2/4/pi
        ...
        scatterlines!(ax,eigval_bin_centers,e["v_projs_time_avg"]["v_projs_time_avg"]/e["v0"]^2, colorrange = (1, 6 ) , color=color_dict[e["Nint"]], label="J = $(e["J"]), Nint = $(e["Nint"])",marker=marker_dict[e["J"]], markersize=10)# ,  label="J = $(e["J"])",alpha=0.3)
        ...
        if J==0.2
        lines!(ax,the_eigvals,theory_integral, color=color_dict[e["Nint"]], colorrange = (1, 6 ) ,  label="J = $(e["J"]) theory a=a_min", linestyle=:dash)
        end
    end
...
xlims!(ax, 0.002,1)
f[1,2]=Legend(f,ax)
save(joinpath(figure_save_folder,"v_projs_tau_J.pdf"), f,backend=CairoMakie)
display(f)#
end
...
for e in ensemble_files
    ...
    ax = Axis(f[1,1], ylabel=L"ω_n", xlabel=L"ω", title="Nint = $(e["Nint"]),J=$J, Dr = $Dr, v0 = $(v0)")
