...
base_folder = "/Volumes/T7_Shield/sa/statistics/hex_disordered/phi_1.3/fa"    #"/Users/kammeraat/mounting/data2_kammeraat/sa/statistics/hex_disordered/phi_1.3/Nlin_20"




figure_save_folder = joinpath(base_folder, "figures_16_02")
...
ensemble_files = ensemble_files[e_ind_sort]
...
for e in ensemble_files
    ...
    scatterlines!(ax, e["vrms_r"]["r_bin_centers"], e["vrms_r"]["vrms_r"]/e["v0"], colorrange = (0, 10 ) , color=color_dict[e["J"]], label="J = $(e["J"])", colormap=:turbo)
    end
...
f[1,2]=Legend(f,ax)
display(f)
save(joinpath(figure_save_folder,"vrms_r.pdf"), f, backend=CairoMakie)
end
...
for e in ensemble_files
    ...
    scatterlines!(ax,det , e["auto_p"]["Cavg"], colorrange = (0, 10 ) , color=color_dict[e["J"]], label="J = $(e["J"])", colormap=:turbo)
...
f[1,2]=Legend(f,ax)
display(f)
save(joinpath(figure_save_folder,"auto_n.pdf"), f, backend=CairoMakie)
...
ax = Axis(f[1,1], xlabel=L"$\Delta t$", ylabel=L"$\langle \vec{v}(t+\Delta t)\cdot \vec{v}(t) \rangle$/(v_{rms}(t+\Delta t)v_{rms}(t))", title="Dr =0.1")
for e in ensemble_files
    ...
    scatterlines!(ax,det , e["auto_v"]["Cavg"], colorrange = (0, 10 ) , color=color_dict[e["J"]], label="J = $(e["J"])", colormap=:turbo)
...
lines!(ax, ensemble_files[1]["auto_v"]["deltat"],exp.(-ensemble_files[1]["auto_p"]["deltat"].*0.1), color="red", label="ABP_theory")

ylims!(ax,-0.5,1)
f[1,2]=Legend(f,ax)
display(f)
save(joinpath(figure_save_folder,"auto_v.pdf"), f, backend=CairoMakie)
...
for e in ensemble_files
    ...
    if Dr==0.1 
       ...
       # theory_integral_2 = theory_integral_2/4/pi
        ...
        scatterlines!(ax,eigval_bin_centers,e["v_projs_time_avg"]["v_projs_time_avg"]/e["v0"]^2, colorrange = (0, 10 ) , color=color_dict[e["J"]], label="J = $(e["J"])", colormap=:turbo)# ,  label="J = $(e["J"])",alpha=0.3)
...
for e in ensemble_files
    ...
    lines!(ax, sqrt.(eigval_bin_centers*(1/tau + J*a^2*v0)), sqrt.(eigval_bin_centers))
...
for Drplot in [0.1]
    ...
    for e in ensemble_files
        ...
        J = e["J"]
        tau = 1/Dr
        if (eigvals[eigval_ind]+1/tau)/(J*v0)<1
            a_min = sqrt(1 -  (eigvals[eigval_ind]+1/tau)/(J*v0))
        else
            a_min = a_ABP
        end
        ...
        tau = 1/Dr
        #a_minse = @. sqrt(-1 +  (eigvals+1/tau)/(J*v0))
