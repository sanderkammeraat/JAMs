...
ensemble_files[12]["v_projs_time_avg"]
...
for e in ensemble_files
    ...
    a_min= @.sqrt(1 +  ( 0+ 1/(2 * tau *J ))^2 ) - ( 0 + 1/(2 * tau *J ))

    hlines!(ax, a_ABP )
    hlines!(ax, a_min, color="red" )
    hlines!(ax, e["vrms"]/e["v0"], color=e["J"], colorrange = (0, 1 ) )
    end
...
ax = Axis(f[1,1], xlabel=L"r", ylabel=L"vrms/v0")
...
for e in ensemble_files
    ...
    hlines!(ax, a_ABP )
    scatter!(ax, e["vrms"]/e["v0"], color=e["J"], colorrange = (0, 1 ) )
    end
...
for e in ensemble_files
    ...
    display(length(w))
    dt = t[2]-t[1]
    scatterlines!(ax,w, sum(e["FT_v_projs"]["X2"]   ,dims=1)[1,:] *dt^2 * (2*pi/length(t[e["min_t_ind"]:end])/dt), colorrange=(1,13), colormap=:gist_rainbow, color=color_dict[J],label="$J",alpha=0.2)

    lines!(ax,w, sum(X ,dims=1)[1,:] , colorrange=(1,13), colormap=:gist_rainbow, color=color_dict[J],label="$J th")
    label =  L"\langle \log_{10}| \dot{a}_{\nu}(\omega) |^2 \rangle"
...
for e in ensemble_files
    ...
    if Dr==0.1 && J>0
        ...
        # e["eigenmodes"]["eigvals"]["seed_2.h5"]
        a_ABP = sqrt(1/e["Nint"]*sum(1 ./(2 .+ 2*tau .* eigvals))) 

        p= [eigval_bin_centers, J, tau,e["v_projs_time_avg"]["v_projs_time_avg"][2]/e["v0"]^2]
...
for e in ensemble_files
    ...
    a = a_min = sqrt(1 +  (0*eigval_bin_centers[eigval_ind]/(2 * J) + 1/(2 * tau *J ))^2 ) - (0*eigval_bin_centers[eigval_ind]/(2 * J) + 1/(2 * tau *J))
    ...
    #f[1,2]=Legend(f,ax)
    save("temp.pdf",f)
    append_pdf!( joinpath(figure_save_folder,"Dre_0.1_wn_w_theory_a_min.pdf"), "temp.pdf", cleanup=true)
